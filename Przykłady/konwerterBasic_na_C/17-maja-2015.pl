...
modul(M,Wci) --> "Module", odstep_k, nazwa(_), nl_k, !, {Wci1 is Wci+1}, modul_cialo(MC,Wci1), !, "End Module", {concat_atom([MC],M)}.
modul_cialo(M,Wci) --> funkcja(F,Wci),   modul_cialo(M2,Wci), {concat_atom([F,M2],M), info(' FUN ')}.
modul_cialo(M,Wci) --> procedura(P,Wci), modul_cialo(M2,Wci), {concat_atom([P,M2],M), info(' PROC ')}.
modul_cialo('',_) --> "".

funkcja(F,Wci) --> funkcja_nagl(FN), {Wci1 is Wci+1}, funkcja_cialo(FC,Wci1),  {info('funkcja\n'), wciecie(W,Wci), concat_atom(['\n',W,FN,'\n',W,'{\n',FC,W,'}\n'],F)}.
...
procedura(F,Wci) --> procedura_nagl(PN), {Wci1 is Wci+1}, procedura_cialo(PC,Wci1), {info('procedura\n'), wciecie(W,Wci), concat_atom(['\n',W,PN,'\n',W,'{\n',PC,W,'}\n'],F)}.
...
lista_zmiennych(L,Wci) --> lista_nazw_zmiennnych(LNZ), odstep_k, "As", odstep_k, typ(T), {wciecie(W,Wci), concat_atom([W, T,' ',LNZ,';'],L),info(L)}.
lista_nazw_zmiennnych(L) --> nazwa(N), odstep, ",", odstep, lista_nazw_zmiennnych(LNZ), {concat_atom([N,', ',LNZ],L)}.
lista_nazw_zmiennnych(L) --> nazwa(N),"(", calkowita(LC), ")", odstep, ",", odstep, lista_nazw_zmiennnych(LNZ), {concat_atom([N,'[', LC, '], ',LNZ],L)}.
lista_nazw_zmiennnych(Z) --> nazwa(N),"(", calkowita(LC), ")", {concat_atom([N,'[', LC, ']'],Z)}.
